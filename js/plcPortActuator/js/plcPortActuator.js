...
class ClassPortActuator extends ClassActuator {
    constructor(opts) {
        ...
        if (Object.keys(this._ChannelNames).length !== this._Pins.length)
            throw new Error('Quantity of channels must be equal to pins count');
        // Тип сигнала определяет команду записи в порт
        if (!Array.isArray(this._TypeOutSignals) || this._TypeOutSignals.length !== Object.keys(this._ChannelNames).length)
