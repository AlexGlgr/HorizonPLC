...
class HorizonAT {
        ...
        this._Serial.on('data', cb = (_data) => {
            ...
            if (this._Handlers) {
                for (let head in this._Handlers) {
                    try {
                        while (this._Line.startsWith(head)) {
                            let pre = this._Line;
    ...
    Execute (_comm, _timeout, callback) {
        // в данный момент выполняется другая команда - отмена, мы не должны доходить до этого
    ...
    RegisterLine (_key, callback) {
        if (this._LineHandlers[_key] !== undefined) return;
    ...
    Register (_key, callback) {
        if (this._Handlers[_key] !== undefined) return;
...
class ESP {
    constructor(bus_serial) {
        ...
        require('NetworkJS').create({
            create : (host, port, socketType, options) => this.Create(host, port, socketType),
            close : (socket) => this.Close(socket),
            accept : (socket) => this.Accept(socket),
            recv : (socket, maxLen) => this.Recv(socket, maxLen),
            send : (socket, data, socketType) => this.Send(socket, data)
        });
       this._SocketArray = [
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined },
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined },
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined },
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined },
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined },
            { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined }// reserved for server
        ];
    }
    /**
     * @method
    ...
    Init(callback) {
        ...
        for(let i = 0; i < MAXSOCKETS; i++) {
            this._At.RegisterLine(`${i},CONNECT`, (ln) => this.socketOpen(ln));
            this._At.RegisterLine(`${i},CLOSED`, (ln) => this.socketClose(ln));
        }
    ...
    Create(host, port, socketType){        
        ...
        if (host === undefined && socketType != 2) {// Create server socket at 6th reserved slot
            let srv = this._SocketArray[MAXSOCKETS].Service;
            this._SocketArray[MAXSOCKETS] = { Status: STATUSES[2], Type: TYPES[2], Data: '', Service: srv };

            const start = Date.now();
            this._At.Execute(`AT+CIPSERVER=1,${port}\r\n`, 10000, (d) => {
                if (d == 'OK') this._SocketArray[MAXSOCKETS].Status = STATUSES[1];
                else {
                    this._SocketArray[MAXSOCKETS] = { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined };
            ...
            let index = 0;
            console.log("Create socket type ",socketType);
            let cmd;
            let srv;
            ...
            srv = this._SocketArray[index].Service;
            if (socketType == 2) {// It's a UDP
                cmd = `AT+CIPSTART=${index},"UDP","${this._SocketArray[index].Service.Host}",${this._SocketArray[index].Service.Port},3600,2\r\n`;
                this._SocketArray[index] = { Status: STATUSES[2], Type: TYPES[3], Data: '', Service: srv };
            }
            else {// It's a TCP
                cmd = `AT+CIPSTART=${index},"TCP","${host}",${port}\r\n`;
                this._SocketArray[index] = { Status: STATUSES[2], Type: TYPES[1], Data: '', Service: srv };
            }
    ...
    Recv(_index, _maxLen) {
        if (this._SocketArray[_index].Data !== undefined) {
            ...
            if (this._SocketArray[_index].Data.length > _maxLen) {
                ...
                r = this._SocketArray[_index].Data;
    ...
    Send(_index, _data) {
        if (this._At.IsBusy() || this._SocketArray[_index].Status == STATUSES[2]) {
            return 0;
        }
        ...
        if (this._SocketArray[_index].Type == TYPES[3]) { // UDP
            _data = _data.substr(8,this.udpToIPAndPort(_data).len);
            returnVal = _data.length + 8;
        }
        
        this._At.Execute(`AT+CIPSEND=${_index},${_data.length}\r\n`, 2000, (d) => {
            if (d == 'OK') {
                this._At.Register('>', (line) => {
                    this._At.Unregister('>');
                ...
                this._SocketArray[_index] = { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined };
                this._At.Unregister('>');
                return false;
            }
            else if (d == "SEND OK") {
                ...
                console.log('Undefined response from sending packet');
            }
            else return true;
        });
    ...
    IpdHandler(line) {
        ...
        if (len>=parms[1]) {
            // we have everything
            this._SocketArray[index].Data += line.substring(colon+1, colon+1+parms[1]);
            return line.substring(colon+1+parms[1]); // return anything else
        } 
        else {
            // still some to get - use getData to request a callback
            this._SocketArray[index].Data += line.substring(colon+1,len);
    ...
    socketOpen(ln) {
        ...
        if (this._SocketArray[index].Status == STATUSES[0] && this._SocketArray[MAXSOCKETS].Type == TYPES[2]) {
            ...
            Object.emit('SOpen',index);
        } 
        else {
            // Otherwise we had an error - timeout? but it's now open. Close it.
            this._At.Execute(`AT+CIPCLOSE=${index}\r\n`,1000, () => {
                this._SocketArray[index] = { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined };
                H.Logger.Service.Log({service: 'ESP', level: 'E', msg: `Unexpected socket opens at slot ${index}. Closed!`});
            });
        }
    }    
    socketClose(ln) {
        this._SocketArray[ln[0]] = { Status: STATUSES[0], Type: TYPES[0], Data: undefined, Service: undefined };
    }
    ...
    Echo(_val) {
        return new Promise((res,rej) => {
            ...
            this._At.Execute('ATE0\r\n', 1000, (d) => {
                if (d == 'OK') res(d);
                else if (d == 'ATE0' || d == 'ATE1') return true;
    ...
    MulSockets(_val) {
        return new Promise((res,rej) => {
            ...
            this._At.Execute(`AT+CIPMUX=${_val}\r\n`, 1000, (d) => {
                if (d == 'OK') res(d);
                else if (d == 'ERROR') rej(`CIPMUX failed. Reason: (${d}) Timeout ${Math.floor((Date.now() - start) / 1000)} second(s)`);
                else return true;
            })
        })
    }
    ...
    Country(_ind, _startch, _totch) {
        return new Promise((res,rej) => {
            if (_startch < 1 || _startch > 14) rej(`CWCOUNTRY failed. Reason: Start channel should be between 1 and 14 (got "${_startch}")`);
            if ((14 - _startch) > _totch) rej(`CWCOUNTRY failed. Reason: Incorrect amount of total channels (got "${_totch}", awailable "${14-_startch}")`);
            const start = Date.now();
    ...
    Connect(_ssid,_pass) {
        return new Promise((res,rej) => {
            ...
            this._At.Execute(`AT+CWJAP="${_ssid}","${_pass}"\r\n`, 20000, (d) => {
                if (["WIFI DISCONNECT","WIFI CONNECTED","WIFI GOT IP","+CWJAP:1"].indexOf(d)>=0) return true;
                else if (d == 'OK') res(d);
                else {
                    let reason = 'UNKNOWN';
   ...
   SetSocketEndpoint(_host, _port, _serv) {
        let index = 0;
        while (this._SocketArray[index].Status != STATUSES[0]) {
            index++;
            if (index > MAXSOCKETS) {
                H.Logger.Service.Log({service: 'ESP', level: 'E', msg: 'No free slots for a socket!'});
                return;
            }
        }
        this._SocketArray[index].Service = {Name: _serv, Host: _host, Port: _port};
    }
    GetStatuses() {
        this._At.Execute('AT+CIPSTATUS\r\n', 1000, (d) => {
            if (d == 'OK')  {
                return false;
            }
            else if (d.startsWith('+CIPSTATUS')) {
                console.log(d.substring(11, d.length));
                return true;
            }
            else return true;
        });
