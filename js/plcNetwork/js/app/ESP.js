...
class HorizonAT {
        ...
        if (_timeout > 0) { // если усановлен таймаут - заряжаем его
            let cbTOut = setTimeout(() => {
                ...
                if (this._LineCallback === undefined && this._Waiting.length > 0) {
                    console.log('LCB: ' + this._LineCallback);
    ...
    ExecuteSend (_index, _data) {
        console.log(this._LineCallback);
        if (this._LineCallback != undefined) {
            this._Waiting.push([_index, _data]);
            console.log('Pushing for '+ _index);
            return;
        }
        else console.log('Popping ' + _index);
        //else console.log ('Sending data: ' + _data);

        this._Serial.write(`AT+CIPSEND=${_index},${_data.length}\r\n`);
        ...
        let response = (d) => {
            ...
            if (d == 'OK') {
                this.Register('>', (line) => {
                    this.Unregister('>');
                    this._Serial.write(_data);
                    console.log ('Sending data: ' + _data);
                    return line.substring(1);
                });
                return response;
            }
            if (d == 'SEND OK') return false;
        ...
        let cbTOut = setTimeout(() => {
            ...
            if (this._Waiting.length > 0) {
                this.ExecuteSend.apply(this, this._Waiting.shift());
            }
        }, 8000);
...
class ESP {
    ...
    Create(host, port, socketType, options){        
        ...
        if (host === undefined && socketType != 2) {// Create server socket at 6th reserved slot
            ...
            if (socketType == 2) {// It's a UDP
                cmd = `AT+CIPSTART=${index},"UDP","${srv.Host}",${srv.Port},3600,2\r\n`;
    ...
    Send(_index, _data, _socketType) {
       ...
       // return this.udpToIPAndPort(_data).length+8;
        ...
        //this._At.ExecuteSend(_index, _data);
