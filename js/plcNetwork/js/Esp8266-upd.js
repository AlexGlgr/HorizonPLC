...
var netCallbacks = {
  ...
  send : function(sckt, data, type) {
    console.log("SEND ",udpToIPAndPort(data));
    if (!at) return -1; // disconnected
...
var wifiFuncs = {
  ...
  /**
   * @function
   * Выполняет указанную пользователем АТ-команду
   * @param {String} comm         - АТ-команда
   * @param {Number} to           - ожидание в милисекундах 
   */
  atcomm : (comm, to) => {
    at.cmd(comm+'\r\n', to?to:1000, (d) => {
      console.log(d);
   ...
   * @param {Function} callback   - колбэк-функция, возвращающая результат работы команды
   */
  restore : () => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+RESTORE\r\n', 10000, cb = (d) => {
      if (d != 'ready') return cb;
      else if (d === undefined) rej(`Restore failed. Reason: Chip didn't respond after ${Math.floor((Date.now() - start) / 1000)} seconds`);
      else res('OK');
    });
  }),
   ...
   * @returns {Promise}
   */
  rst : () => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('\r\nAT+RST\r\n', 10000, cb = (d) => {
      if (d == 'ready') res('OK');
      else if (d === undefined) rej(`Reset failed. Reason: Chip didn't respond after ${Math.floor((Date.now() - start) / 1000)} seconds`);
      else return cb;
    });
  }),
   ...
   * @param {Number} val          - значение 0 или 1
   * @returns {Promise}
   */
  echo : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd("ATE0\r\n", 1000, cb = (d) => {
      if (d == 'OK') res(d);
      else if (d == 'ATE0' || d == 'ATE1') return cb;
      else rej(`ATE failed. Reason: ${d?`Value should be 0 or 1 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {Number} val          - значение 0 - 3
   * @returns {Promise}
   */
  sleep : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+SLEEP='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`SLEEP failed. Reason: ${d?`Value should be between 0 and 3 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @returns {Promise}
   */
  sysStore : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+SYSSTORE='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`SYSSTORE failed. Reason: ${d?`Value should be 0 or 1 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {Number} val          - значение 1, 3 или 7
   * @returns {Promise}
   */
  staProto : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWSTAPROTO='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CWSTAPROTO failed. Reason: ${d?`Value should be 1, 3 or 7 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @returns {Promise}
   */
  mulSockets : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CIPMUX='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CIPMUX failed. Reason: ${d?([0,1].includes(val)?`AT+CIPMODE and AT+CIPSERVER should be off`:`Value should be 0 or 1 (got "${val}")`):`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @function
   * Настройка страны
   * @param {String} ind            - код страны
   * @param {Number} startch        - номер начального канала (от 1 до 14)
   * @param {Number} totch          - общее количество каналов
   * @returns {Promise}
   */
  country : (ind, startch, totch) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWCOUNTRY=1,\"'+ind+'\",'+startch+','+totch+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CWCOUNTRY failed. Reason: ${d?([1,14].includes(startch)?`Start channel should be between 1 and 14 (got "${startch}")`:((14-startch)>totch?`Incorrect amount of total channels (got "${totch}", awailable "${14-startch}")`:'Unknown error. Double check country code.')):`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @returns {Promise}
   */
  cipdInfo : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CIPDINFO='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CIPDINFO failed. Reason: ${d?`Value should be 0 or 1 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {Number} val          - целое значение между 2048 и 4096
   * @returns {Promise}
   */
  sslSize : (val) => new Promise((res, rej) => {
    at.cmd('AT+CIPSSLSIZE='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CIPSSLSIZE failed. Reason: ${d?`Buffer size should be between 2048 and 4096 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {Number} val          - целое значение между 1 и 3
   * @returns {Promise}
   */
  mode : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWMODE='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CWMODE failed. Reason: ${d?`Value should be 1,2 or 3 (got "${val}")`:`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {String} ssid       - SSID сети, к которой подключаемся
   * @param {String} pass       - пароль выбранной сети
   * @returns {Promise}
   */
  connect : (ssid, pass) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWJAP=\"'+ssid+'\",\"'+pass+'\"\r\n', 20000, cb = (d) => {
   ...
   * @returns {Promise}
   */
  autoConnect : (val) => new Promise((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWAUTOCONN='+val+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej(`CWAUTOCONN failed. Reason: ${d?([0,1].includes(val)?`AT+CWMODE should be 1`:`Value should be 0 or 1 (got "${val}")`):`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),
   ...
   * @param {Number} interval      - интервал между попытками в секундах. Макс - 7200, 0 - функция отключена. 
   * @param {Number} retries       - количество попыток переподключения. Макс - 1000, 0 - бесконечно пытаться. 
   * @returns {Promise}
   */
  reConnect : (interval, retries) => new Promise ((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CWRECONNCFG='+interval+','+retries+'\r\n', 1000, (d) => {
      if (d == 'OK') res(d);
      else rej (`CWRECONNCFG failed. Reason: ${d?([0,7200].includes(interval)?`Interval should be whole number between 0 and 7200 (got "${interval}")`:([0,1000].includes(retries)?`Retires should be whole number between 0 and 1000 (got "${retries}")`:'Unknown error')):`Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`}`);
    });
  }),

  /**
   * @function
   * Возвращает статус чипа, а также информацию об установленных соединениях.
   * Коды статусов:
   * 2 - ESP8266 подключено к сети и уже получило IP адрес;
   * 3 - Существует активное TCP/UDP соединение;
   * 4 - Недавнее TCP/UDP соединение разорвано;
   * 5 - ESP8266 не подключено ни к какой сети.
   */
  connStatus: () => {
    at.cmd('AT+CIPSTATUS\r\n', 1000, (d) => {
      print(d);
   ...
   * @returns {Promise}
   */
  getIP : () => new Promise ((res, rej) => {
    const start = Date.now();
    at.cmd('AT+CIFSR\r\n', 1000, cb = (d) => {
      if (d == 'OK') {return cb;}
      else if (d.startsWith('+CIFSR:STAIP')) {res(d.substring(14,d.indexOf('"',15)));}
      else if (d === undefined) {rej (`Not connected to any access points`);}
      else {rej (`CIFSR? failed. Reason: Timeout ${Math.floor((Date.now() - start) / 1000)} seconds`);}
    });
  }),

  /**
   * @function
   * Инициализация модуля для работы в режиме станции
   * @param {Function} callback   - функция, выполняемая при успешном завершении инициализации 
   */
  init : (callback) => {
    wifiFuncs.rst()
    .then(() => wifiFuncs.echo(0))
    .then(() => wifiFuncs.mulSockets(1))
    .then(() => wifiFuncs.cipdInfo(1))
    .then(() => wifiFuncs.sleep(0))
    .then(() => {callback(null);})
    .catch((err) => {
      callback(err);
    })
...
exports.setup = function(usart) {
  netCallbacks.on('err', (e) => { H.Logger.Service.Log({service: 'ESP', level: 'E', msg: e}); });
  ...
  at.registerLine("WIFI CONNECTED", () => { H.Logger.Service.Log({service: 'ESP', level: 'I', msg: 'Assotiated.'}); });
  at.registerLine("WIFI GOT IP", () => { H.Logger.Service.Log({service: 'ESP', level: 'I', msg: 'Connected.'}); Object.emit('connect'); });
  at.registerLine("WIFI DISCONNECTED", () => { H.Logger.Service.Log({service: 'ESP', level: 'I', msg: 'Disconnected'}); Object.emit('disconnect'); });
