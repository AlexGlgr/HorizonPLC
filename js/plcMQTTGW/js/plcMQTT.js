...
class ClassMQTTOld {
    constructor(_options) {
        ...
        this.port = _options.port || C.DEF_PORT;
        this.client_id = _options.client_id || mqttUid();
        this.keep_alive = _options.keep_alive || C.DEF_KEEP_ALIVE;
        this.clean_session = _options.clean_session || true;
        ...
        this.protocol_name = _options.protocol_name || "MQTT";
...
class ClassMQTT {
    constructor(_options) {
        ...
        Object.on('SOpen', ind => {
            if (this.client && this.client.sckt-1 == ind) {
                console.log('SOpen');
                if (this.connected) 
                    H.Logger.Service.Log({ service: 'MQTT', level: 'I', msg: `Socket ${ind} opened but already connected` });
                this.onConnect(this.client);
            }
        });
    }
    // Handle a single packet of data
    packetHandler(data) {
        console.log(JSON.stringify(stringToHexArray(data)));
        // if we had some data left over from last
        // time, add it on
        let partDataCopy = '';
        if (this.partData) {
            data = this.partData + data;
            partDataCopy = this.partData+'';
            this.partData = '';
        }
        // Figure out packet length...
        var dLen = mqttPacketLengthDec(data.substr(1, 5));
        ...
        if (type === TYPE.PUBLISH) {
            ...
            if (RETURN_CODES[returnCode] === 'ACCEPTED') {
                ...
                H.Logger.Service.Log({ service: 'MQTT', level: 'I', msg: `Unsupported packet`});//=${JSON.stringify(stringToHexArray(data))} partData=${partDataCopy}` });
            }
        } else {
            // this.emit('error', "MQTT unsupported packet type: " + type);
            H.Logger.Service.Log({ service: 'MQTT', level: 'I', msg: `Unsupported packet`});//=${JSON.stringify(stringToHexArray(data))} partdata=${partDataCopy}` });
    ...
    onConnect(client) {
        H.Logger.Service.Log({ service: 'MQTT', level: 'I', msg: `Connect req sent`});
        // write connection message
        client.write(this.mqttConnect(this.client_id));
        // handle connection timeout if too slow
        this.ctimo = setTimeout(() => {
    ...
    connect() {
        if (this.client) {
            ...
            try {
                client = require("net").connect({ host: this.host, port: this.port }, _socket => {
                    this.client = _socket;
                });
            } catch (e) {
                this.client = false;
